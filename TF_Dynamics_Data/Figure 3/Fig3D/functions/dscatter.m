...
if nargin > 2
    ...
    okargs = {'smoothing','bins','plottype','logy','contourFlag','marker','msize','filled'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
elseif length(k)>1
    ...
    switch(k)
        ...
        case 5 %'scatter'
            % Change color map of dscatter
            colormap('jet')
%             temp = [.2:.01:1]';
%             targetcolor = [43,140,190]./255;
%             cmap = zeros(numel(temp),3);
%             cmap = [temp*targetcolor(1),temp*targetcolor(2),temp*targetcolor(3)];
%             colormap(cmap)
