% declare 
imagedir = 'Y:\michael\';
experimentdir = [imagedir,'TF dynamics data\Figure S6\FigS6G_S6H\'];
datadir = [experimentdir,'Data\'];
% declare row, col, and sites to analyze
rows = [2]; numrows = numel(rows);
cols = [5:7]; numcols = numel(cols);
...
%             
            ...
            dscatter(ppargifwell,yfptagwell)
            set(gca,'YLim',[0 350],'XLim',[0 350])
            axis square
            xlabel 'PPARG-IF'
            ylabel 'PPARG-citrine'
%             figure(plate+numplates)
%             subtightplot(numrows,numcols,posmat(row,col),[0.05 0.05],.1,.1)
%             histogram(ppargifwell)
%             set(gca,'XLim',[0 300])
%             if cols(col)<8
%                 dscatter(ppargifwell,yfptagwell)
%             else
...
%             end
%             set(gca,'YLim',[0 300],'XLim',[0 300],'FontName','Arial','FontSize',20)
        end
...
IF = squeeze(ppargmean);
figure,barwitherr([std(IF);std(citrine)]',[mean(IF);mean(citrine)]')
legend({'IF signal','Citrine Signal'},'Location','NorthWest')
