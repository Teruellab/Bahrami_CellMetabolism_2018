...
experimentpath='TF dynamics data\Fig4C\';
...
load([datadir,siName{sin},'_CombinedWithLinking.mat'])
...
framesperhr = 6;
drugspike = 0; %114
spikeTime = [286]./framesperhr; %,142,205,284,352,424,493
...
gatein = gatestartpoint & gateendpoint & ~highcells;
...
lowcells = lowcells(gatein);
numgated = length(allIDs);

% plot median + percentile bounds
tracematrix = alltraces1(lowcells,:);

colorchoice = colorchoices(sin,:);
xtime=((1:endframe))/framesperhr;
...
set(gca,'XLim',[0 70],'XTick',0:12:72,'YLim',[0 80]);