...
switch maxpos
    ...
    case 1
        for i=1:numtraces
            toohigh(i)=signals(i,tracestats(i,1))>minthreshold;
        end
    case 2
        for i=1:numtraces
            toohigh(i)=signals(i,tracestats(i,2))>minthreshold;
        end
end
...
for i=1:numtraces
%     smoothsig = medsmoothignorenans(signals(i,:),5);
%     maxdiff = max(abs(diff(smoothsig)./smoothsig(1:end-1)));
