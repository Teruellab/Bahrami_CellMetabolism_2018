...
imagepath = 'z:\michael\';
experimentpath='Live Cell Experiments\20170725-MZ\';
shot=[num2str(row),'_', num2str(col), '_', num2str(site)];
wellname = nameandsite(shot,1);
...
badtraces= badlengths ; %| badtraces2; %|  badIF | orphan; %badtraces1 | badtraces2;% | badtraces2;
...
% 
% highcells = zeros(numgated,1);
% for cellID = 1:numgated;
%     maxvalue = max(traces1(cellID,find(time==0):end));
%     highcells(cellID) = maxvalue>0.7;
% end
% 
% predictedcells = zeros(numgated,1);
% for cellID = 1:numgated;
%     maxvalue = max(traces1(cellID,time==0+24));
%     predictedcells(cellID) = maxvalue>0.35;
% end
% highcells= predictedcells;
% disp(sum(highcells)*100/length(highcells))
%%% plotting %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     if highcells(counter) == 1
        tracecolor = [0 0 1];
%     else
%         tracecolor = [1 0 0];
%     end
    if ~plotsignal2
        line(xtime,ysig1,'DisplayName',num2str(i),'linewidth',tracewidth,'Color',tracecolor);
