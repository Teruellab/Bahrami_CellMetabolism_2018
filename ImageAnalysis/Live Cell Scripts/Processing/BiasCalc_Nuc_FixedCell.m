...
%Mark if files from multiple time points are stored seaparately or together
...
for i=1:length(names)
    ...
    parfor s=1:numsites
        site=sitemat(s);
%         biasstack=[];
        nucbiascalculator(imagepath,experimentpath,channelName,biasdir,height,width,bgcmos,nucr,blobthreshold,debrisarea,site,rowmat,colmat,framemat,separatedirectories)
%         for c=1:numcols
%             col=colmat(c);
%             for r=1:numrows
%                 row=rowmat(r);
...
%         %%%%for immunostain
%         save([biasdir,names{i},num2str(site),'.mat'],'bias');
