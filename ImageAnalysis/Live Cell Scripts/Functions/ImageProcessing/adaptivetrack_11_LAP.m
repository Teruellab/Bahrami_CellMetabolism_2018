function [tracedata,curdatatracked,tracking,nuc_label]=adaptivetrack_10_LAP(tf,lastgoodframe,curframe,tracedata,curdata,tracking,nuc_raw,nuc_label,nucr,jitter,maxjump,debrisarea,H2BvsNLS,debugpackage)
%masschangethreshold=0.20; %MCF10A-10xBin1=0.20  BJ5-10xBin2=0.10
masschangethreshold = 0.2;
areachangethreshold = 0.2;
if H2BvsNLS==1
    daughtermin=-0.55;
    daughtermax=-0.45;
elseif H2BvsNLS==2
...
%%% set up %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% winrad=4*nucr; %prev:3*nucr
% previous frame information
maxcellnum=size(tracedata,1);
numprevtotal=find(~isnan(tracedata(:,lastgoodframe,1)),1,'last'); % number of previous cells (total, including ones that have been lost)
xprev=tracedata(1:numprevtotal,lastgoodframe,1); % x position of previous cells
yprev=tracedata(1:numprevtotal,lastgoodframe,2); % y postion of previous cells
areaprev=tracedata(1:numprevtotal,lastgoodframe,3); % area of previous cells
massprev=tracedata(1:numprevtotal,lastgoodframe,4); % mass of previous cells
previd=find(~isnan(xprev));
numprevextant=numel(previd); % the number of previous existing cells that was successfully tracked (not equal to numprevtotal)

% current frame information(x and y coordinates, numcells, merges)
...
shortmerge=(curframe-tracking(:,4))<=6; %was 5 before
...
% Look through all of the previous cells
...
%     end
    ...
    if massdiff > masschangethreshold || ongoingmerge(p)
        ...
        %tempx=rxcur(match);
        %tempy=rycur(match);
        %denovo_id=denovo_lab+el(tempy,tempx);
        %if denovo_id==0
...
%         [bordermask,borderflag(match)]=splitdeflections_5_bwboundaries(orderedset,bordermask,nucr);
    end
end
...
% LAP linker will link cells 1 to 1 based on distance and total H2B intensity (merges and splits will be excluded)
% only links from previous frame cells to current frame cells (not total past cells)
assignment = LAPlinker(maxjump,masschangethreshold,xprev(previd),yprev(previd),massprev(previd),xcur,ycur,masscur);
lapprev = assignment(1:numprevextant); % column vector matching previous cell to current cell or no match
lapcur = assignment(numprevextant+1:end); % column vector with indices indicating whether a current cell has a match or no match
goodmatchlogical = lapprev<=numcur; % logical marking previous cells 
prevmatchind = previd(goodmatchlogical); % find indices of all previous cells that have been matched to a current cell
unmatchedprev= previd(~goodmatchlogical); % find indices of all previous cells that have not been matched to a current cell
numUnmatch = numel(unmatchedprev); % number of unmatched cells from previous frame

prevmatch(prevmatchind) = lapprev(goodmatchlogical);
curmatch(lapprev(goodmatchlogical)) = 1;

unmatchedcur = lapcur(lapcur<numcur); % current frame cells with no match

% prevmatch(prevmatchind) = assignment(prevmatchind);
% curmatch(assignment(prevmatchind)) = 1;
% Search through unmatched cells to assign cell division, merges, or new tracks
for i=1:numUnmatch
    
    p = unmatchedprev(i);
    neighbors = intersect(find(abs(xcur-xprev(p))<maxjump & abs(ycur-yprev(p))<maxjump),unmatchedcur); 
...
%     end
    ...
    splitcheck=ongoingmerge(p) & shortmerge(p) & abs(sum(massdiff)+1)<masschangethreshold; % to check if an ongoing merge was successfully split by above method
    ...
    elseif any(abs(massdiff)<masschangethreshold) 
...
extractmask = debugpackage{1}; reljitx = debugpackage{3}(:,1);reljity = debugpackage{3}(:,2);
[height,width]=size(nuc_label); winrad = 32;
