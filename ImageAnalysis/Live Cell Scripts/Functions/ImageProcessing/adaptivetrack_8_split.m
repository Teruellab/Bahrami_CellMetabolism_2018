...
masschangethreshold=0.2; % was all 0.2 before
areachangethreshold=0.2; % was 0.2 before
if H2BvsNLS==1
    daughtermin=-0.55;
    daughtermax=-0.45;
elseif H2BvsNLS==2
...
shortmerge=(curframe-tracking(:,4))<=20; %was 5 before
...
%         keyboard;
%     end
    distvec = sqrt((xcur-xprev(p)).^2+(ycur-yprev(p)).^2);
    neighbors = find(distvec<maxjump);
...
%     dist=sqrt((xcur(neighbors)-xprev(p)).^2+(ycur(neighbors)-yprev(p)).^2);
    ...
    celldist=celldist(candidateidx);
    ...
    daughtercheck=~ongoingmerge(p) & massdiff>daughtermin & massdiff<daughtermax & celldist<maxjump & areadiff<0; %H2B: -0.55 to -0.45; NLS: -0.70 to -0.30
    if splitcheck
    ...
    %      ~ongoingmerge(p) && areadiff(1)>debrisarea && areadiffnorm(1)>areachangethreshold
