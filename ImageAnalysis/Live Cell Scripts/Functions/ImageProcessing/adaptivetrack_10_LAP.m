...
assignment = LAPlinker(maxjump,masschangethreshold,xprev,yprev,massprev,xcur,ycur,masscur);

lapprev = assignment(1:numprevextant); 
lapcur = assignment(numprevextant+1:end);

prevnotmatch = lapprev>numcur; % previous frame cells with no match
prevmatchind = find(lapprev<=numcur); % index of previous frame cells with match 
unmatchedprev = find(prevnotmatch); % index of previous frame cells with no match
numUnmatch = numel(unmatchedprev);

curnotmatch = lapcur<numcur;
unmatchedcur = lapcur(curnotmatch); % current frame cells with no match
