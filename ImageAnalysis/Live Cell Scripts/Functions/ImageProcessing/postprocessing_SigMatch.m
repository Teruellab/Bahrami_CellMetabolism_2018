function [tracedata,genealogy,jitters]=postprocessing_SigMatch(tracedata,cellid,jitters,badframes,tracking,maxcellnum,nucr)
...
omass=ones(numorph,1)*NaN; ox=omass; oy=omass; osig = omass;
for i=1:numel(orphan)
    omass(i)=tracedata(orphan(i),ostart(i),4); 
    ox(i)=tracedata(orphan(i),ostart(i),1); 
    oy(i)=tracedata(orphan(i),ostart(i),2);
    osig(i)=tracedata(orphan(i),ostart(i),3).*tracedata(orphan(i),ostart(i),6);
end
% winrad=round(3*nucr);
winrad=24;
for i=1:numel(fin)
    tmass=tracedata(fin(i),finend(i),4);
    tx=tracedata(fin(i),finend(i),1); 
    ty=tracedata(fin(i),finend(i),2); 
    tsig = tracedata(fin(i),finend(i),3)*tracedata(fin(i),finend(i),6);
%     if tsig>1.5e4
        critsigval = .15;
%     else
%         critsigval = -5.667e-5*tsig + 1;
%     end
    c=find(ostart-finend(i)<=10 & ostart-finend(i)>0 & abs(ox-tx)<winrad & abs(oy-ty)<winrad & abs((omass-tmass)/tmass)<0.2 & abs((osig-tsig)/tsig)<critsigval);
    if numel(c)>1
        c=c(find(ostart(c)==min(ostart(c)),1)); %multiple hits, picks one with closest start frame
